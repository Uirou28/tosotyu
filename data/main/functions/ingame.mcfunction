...
execute if score 合計ポイント point = @e[type=armor_stand,tag=game,limit=1] setpoint run function victory:escaper
...
execute as @e[type=armor_stand,tag=game,scores={players=0}] run kill @e[type=armor_stand,tag=game]
