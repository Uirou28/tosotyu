...
effect give @a[team=hunter] mining_fatigue 60 5 true
...
tellraw @a[team=escaper] [{"text":"ハンターに殺される前にエンダードラゴンを倒そう!!","bold":true}]
tellraw @a[team=hunter] [{"text":"エンダードラゴンを倒される前に全員殺そう!!","bold":true}]
...
tag @e[type=armor_stand,tag=game] add dragon
tag @a remove dragon